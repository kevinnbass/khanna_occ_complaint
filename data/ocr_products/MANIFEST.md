...
| `khanna_ptr_corpus.json` | corpus index for `OCC_M013` / `OCC_M014` (late-filing audit + per-tx framing) | Counts 1-3 + 5 + 6 | 114 PTR PDFs 2017-2026 | per-doc URLs at `https://disclosures-clerk.house.gov/public_disc/ptr-pdfs/{year}/{doc_id}.pdf` |
| `khanna_ptrs/<doc_id>/<doc_id>_OCR.txt` | per-page Gemini OCR transcription of every cited PTR (114 docs / 1,787 pages) | Counts 1-3 + 5 + 6 (grep / browse aid) | same 114 PTRs as `khanna_ptr_corpus.json` | per-doc URLs at `https://disclosures-clerk.house.gov/public_disc/ptr-pdfs/{year}/{doc_id}.pdf` |
...
| `rebuild_ptr_audit_khanna.py` | regenerates `ptr_filing_audit_khanna_REBUILT.json` (B-F1) | Counts 1-3 + 5 + 6 reviewer cookie-cutter | (rebuilds from `../occ/khanna_ptr_transactions_2026_05_02.json`) | n/a (stdlib-only Python; no fetch) |
| `rebuild_pfd_schedule_d_khanna.py` | regenerates `pfd_schedule_d_khanna_REBUILT.json` (B-F2) | Count 3 + Count 6 (TY2017 $1M+ Goldman SP single-line anchor) | (rebuilds from `../occ/khanna_pfd_schedule_d_2026_05_02.json`) | n/a (stdlib-only Python; no fetch) |
